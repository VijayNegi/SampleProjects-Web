...
for (let node = 0; node < NODE_COUNT; node++) {
  for (let vnode = 0; vnode < VNODE_COUNT; vnode++) {
    const hsh = crypto.createHash('md5').update(node.toString()).digest().readUInt32BE(0);
    const index = bisectLeft(hash2index, hsh);
    if (index >= hash2index.length) {
      hash2index.push(hsh);
      index2node.push(node);
    } else {
      hash2index.splice(index, 0, hsh);
      index2node.splice(index, 0, node);
    }
  }
}
...
for (let data_id = 0; data_id < DATA_ID_COUNT; data_id++) {
  ...
  const hsh = crypto.createHash('md5').update(data_id_str).digest().readUInt32BE(0);
  let index = bisectLeft(hash2index, hsh);
  const node_ids = [index2node[index]];
  const zones = [node2zone[node_ids[0]]];
  node_counts[node_ids[0]]++;
  zone_counts[zones[0]]++;
  for (let replica = 1; replica < REPLICAS; replica++) {
    while (
      node_ids.includes(index2node[index]) &&
      zones.includes(node2zone[index2node[index]])
    ) {
      index++;
      if (index >= hash2index.length) {
        index = 0;
    ...
    node_ids.push(index2node[index]);
